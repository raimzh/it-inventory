...
test('имя созданной копии проходит собственную проверку сервиса', async () => {
  // Ради этого случая набор и писался. Выражение не включало букву Z,
  // которой заканчивается метка времени из toISOString, — и не совпадало
  // ни с одним реально созданным файлом. На боевом стенде это выглядело
  // так: 18 копий на диске, в списке ноль, скачать нельзя ни одну,
  // старые не удаляются. Заметно только тогда, когда копия понадобилась
  assert.ok(ctx.created.length > 0, 'предыдущий тест должен был создать копию');
  for (const name of ctx.created) {
    assert.match(name, /^backup-[0-9TZ:.-]+\.sql(\.enc)?$/, `имя «${name}» должно подходить под формат`);
    assert.ok(name.includes('Z'), 'метка времени содержит Z — выражение обязано это учитывать');
  }
});
