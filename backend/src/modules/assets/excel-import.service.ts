...
export class ExcelImportService {
  ...
  async generateTemplate(): Promise<Buffer> {
    ...
    [
      ...
      ['Первонач. стоимость', 'Нет', 'Первоначальная стоимость в тенге'],
      ['Остат. стоимость', 'Нет', 'Остаточная стоимость в тенге'],
  ...
  private mapRow(raw: Record<string, any>): Partial<Asset> & { _error?: string } {
    ...
    // Серийный номер, дословно повторяющий наименование, — не серийный
    // номер, а следствие того, что в выгрузке колонки перепутаны. Так в
    // учёт попали 380 записей, где серийником значилось название модели:
    // поиск по серийнику выдавал случайную позицию из десятка одинаковых,
    // а в указателе сканирования такие значения перекрывали друг друга —
    // двенадцать антенн одной модели делили один ключ.
    //
    // Отбрасываем молча: настоящий серийный номер совпасть с
    // наименованием не может, и сообщать оператору здесь не о чем.
    if (dto.serialNumber && dto.name && dto.serialNumber === dto.name) {
